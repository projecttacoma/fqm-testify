...
export function getDataRequirementsLookupKey(dataRequirement: fhir4.DataRequirement, valueSetMap: ValueSetsMap) {
  return `${dataRequirement.type}|${getDataRequirementFiltersString(dataRequirement, valueSetMap)}`;
}
  ...
  get: async ({ get }) => {
    ...
    if (dataRequirements !== null && valueSetMap !== null) {
      dataRequirements.forEach(dr => {
        const key = getDataRequirementsLookupKey(dr, valueSetMap);
        result[key] = dr;
      });
    }
    return result;
